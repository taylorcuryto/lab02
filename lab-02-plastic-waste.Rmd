...
console pane - where you can enter commands or see the outputs
sourse pane - where you write and edit your code
environment/history pane - displays all the comands ran
files/plots/packages/viewer pane - has all the files apart of the repository, view plots created, manage the packages, nd displays the content
...
The observation doesn't have a known value for those specific variables
...
Did you expect this result? No, I am not very familiar with this country, I would have assumed it was USA.
...
The plastic waste per capita for all the areas are very similar from one another

```{r plastic_waste_per_cap-hist2, eval=TRUE}
ggplot(data = plastic_waste, aes(x = plastic_waste_per_cap)) +
...
```{r plastic_waste_per_cap-dens-color-fill-alpha2}
ggplot(data = plastic_waste, 
       mapping = aes(x = plastic_waste_per_cap, 
                     color = continent, 
                     fill = continent)) +
...
The alpha value is not directly tied to the data. color and fill are equal to variables in the dataset while the alpha value is equal to numbers that are not tied to the variables in any way.
...
In a violin plot, you can see where majority of the data is based on how wide each section is. In a box-plot, you can clearly tell where the median, first quartile range, and third quartile range are.
...
It seems like more of a positive relationship. There is one major outlier, but it has a very steep slope.
...
Not really, I think the colors are pretty intermixed and there are little clusters of the same color but they are very close together.
...
Not really by much, but the coastal population has more points scattered upward to give it more of a linear look. They are both not that linear though.
...
Most of the data based on the continents are scattered within each other, so there is no correlation within the continents. Coastal population proportion and plastic waste per capita have somewhat of a positive relationship but has a very weak correlation.
